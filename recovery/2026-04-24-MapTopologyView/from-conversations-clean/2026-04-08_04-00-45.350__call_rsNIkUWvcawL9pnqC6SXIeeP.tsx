...
const MapTopologyView: React.FC = () => {
    ...
    useEffect(() => {
        if (steadyOverlay) {
            setStatusMsg(`æ­£å¼ä»¿çœŸå·²åˆ‡åˆ?${steadyOverlay.scenario_id}ï¼Œrun_id=${steadyOverlay.run_id.slice(0, 12)}`)
        }
    }, [steadyOverlay])
    useEffect(() => {
        if (steadySimError) {
            setStatusMsg(`æ­£å¼ä»¿çœŸå¤±è´¥ï¼?{steadySimError}`)
        }
    }, [steadySimError])
    ...
    const isRuntimeReadonlyGroup = useCallback((group: JunctionGrou…28759 tokens truncated…</div></div><div className="rounded-lg bg-black/15 px-3 py-2"><div className="text-[10px] text-gray-500">å¾…è¡¥è·‘åœºæ™?/div><div className="mt-1 font-semibold text-amber-300">{trialCoverageSummary.pendingCount}</div></div></div><button onClick={async () => { try { await runSteadyMissingTrialScenarios(); setStatusMsg('å·²è¡¥è·‘æ‰€æœ‰æœªç•™æ¡£çš„å…¸åž‹åœºæ™?) } catch (error) { setStatusMsg(error instanceof Error ? error.message : 'æ‰¹é‡è¡¥è·‘å¤±è´¥') } }} disabled={steadyBulkTrialRunActive || steadyTrialRunItems.length === 0 || trialCoverageSummary.pendingCount === 0} className="w-full rounded-lg bg-amber-700/80 px-3 py-2 text-xs font-medium text-white disabled:opacity-40">{steadyBulkTrialRunActive ? 'è¡¥è·‘ä¸?..' : 'ä¸€é”®è¡¥è·‘ç¼ºå¤±åœºæ™?}</button><div className="space-y-2">{steadyTrialRunItems.map(item => { const isRunning = steadyTrialRunScenarioId === item.scenario_id; return <div key={`trial-item-${item.scenario_id}`} className="rounded-lg bg-black/15 px-3 py-2"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="truncate text-gray-100">{item.label}</div><div className="mt-1 text-[10px] text-gray-500">{item.covered ? `å·²ç•™æ¡?${item.snapshot_count} æ¡` : 'è¿˜æ²¡ç•™æ¡£'}</div><div className="mt-1 text-[10px] text-gray-500">æœ€è¿‘æ—¶é—´ï¼š{item.last_saved_at ? formatDateTimeLabel(item.last_saved_at) : '-'}</div></div><button onClick={async () => { try { await runSteadyTrialScenario(item.scenario_id); setStatusMsg(`å·²è¡¥è·‘åœºæ™¯ï¼š${item.label}`) } catch (error) { setStatusMsg(error instanceof Error ? error.message : 'å•åœºæ™¯è¡¥è·‘å¤±è´?) } }} disabled={isRunning || steadyBulkTrialRunActive} className={`rounded-lg px-3 py-1 text-[10px] font-semibold ${item.covered ? 'border border-emerald-400/20 bg-emerald-500/10 text-emerald-100' : 'border border-amber-400/20 bg-amber-500/10 text-amber-100'} disabled:opacity-40`}>{isRunning ? 'è¿è¡Œä¸?..' : item.covered ? 'é‡è·‘' : 'è¡¥è·‘'}</button></div></div> })}</div></div>
                                <div className="rounded-lg border border-emerald-500/20 bg-emerald-950/10 p-3 space-y-3 text-[11px] text-gray-300"><div className="flex items-center justify-between gap-3"><div><div className="text-[10px] uppercase tracking-wider text-emerald-200">è¦†ç›–ç»“æžœæ¸…å•</div><div className="mt-1 text-gray-400">è¿™é‡Œçœ‹å½“å‰åœºæ™¯èƒ½ä¸èƒ½ç›´æŽ¥è®²ã€è¿˜ç¼ºä»€ä¹ˆã€ä¸‹ä¸€æ¡å»ºè®®è®²å“ªä¸ªåœºæ™¯ã€?/div></div><div className={`rounded-full px-3 py-1 text-[10px] font-semibold ${coverageResultSummary.readinessLabel === 'å¯å®Œæ•´æ¼”ç¤? ? 'border border-emerald-400/30 bg-emerald-500/15 text-emerald-100' : coverageResultSummary.readinessLabel === 'å¯å±€éƒ¨æ¼”ç¤? ? 'border border-amber-400/30 bg-amber-500/15 text-amber-100' : 'border border-red-400/30 bg-red-500/15 text-red-100'}`}>{coverageResultSummary.readinessLabel}</div></div><div className="grid grid-cols-2 gap-2 text-xs"><div className="rounded-lg bg-black/15 px-3 py-2"><div className="text-[10px] text-gray-500">å½“å‰åœºæ™¯ç•™æ¡£</div><div className={`mt-1 font-semibold ${coverageResultSummary.currentScenarioCovered ? 'text-emerald-300' : 'text-amber-300'}`}>{coverageResultSummary.currentScenarioCovered ? `å·²ç•™æ¡?${coverageResultSummary.currentScenarioSnapshotCount} æ¡` : 'è¿˜æ²¡ç•™æ¡£'}</div></div><div className="rounded-lg bg-black/15 px-3 py-2"><div className="text-[10px] text-gray-500">æœ€è¿‘è¦†ç›–æ—¶é—?/div><div className="mt-1 font-semibold text-white">{coverageResultSummary.latestCoverageTime ? formatDateTimeLabel(coverageResultSummary.latestCoverageTime) : 'æš‚æ— '}</div></div><div className="rounded-lg bg-black/15 px-3 py-2"><div className="text-[10px] text-gray-500">å½“å‰å‘½ä¸­èŠ‚ç‚¹</div><div className="mt-1 font-semibold text-white">{coverageResultSummary.matchedNodeText}</div></div><div className="rounded-lg bg-black/15 px-3 py-2"><div className="text-[10px] text-gray-500">å½“å‰å‘½ä¸­è¿žçº¿</div><div className="mt-1 font-semibold text-white">{coverageResultSummary.matchedEdgeText}</div></div></div><div className="rounded-lg bg-black/15 px-3 py-2 leading-5 text-gray-300"><div>å»ºè®®æ¼”ç¤ºï¼š{recommendedCoverageScenario?.label ?? 'æš‚æ— '}</div><div>å½“å‰åˆ¤æ–­ï¼š{coverageResultSummary.riskSummary}</div></div><div className="space-y-2">{coverageChecklistItems.map(item => <div key={`coverage-check-${item.scenario_id}`} className="rounded-lg bg-black/15 px-3 py-2"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="truncate text-gray-100">{item.label}</span>{item.isCurrent && <span className="rounded-full border border-sky-400/20 bg-sky-500/10 px-2 py-0.5 text-[10px] text-sky-100">å½“å‰åœºæ™¯</span>}{item.isRecommended && <span className="rounded-full border border-emerald-400/20 bg-emerald-500/10 px-2 py-0.5 text-[10px] text-emerald-100">å»ºè®®å…ˆè®²</span>}</div><div className="mt-1 text-[10px] leading-5 text-gray-400">å…³æ³¨ç‚¹ï¼š{item.focus}</div><div className="text-[10px] leading-5 text-gray-500">é€šè¿‡æ€Žä¹ˆçœ‹ï¼š{item.success}</div><div className="text-[10px] leading-5 text-gray-500">çŽ°åœºé£Žé™©ï¼š{item.risk}</div></div><div className={`rounded-full px-2 py-1 text-[10px] font-semibold ${item.covered ? 'border border-emerald-400/20 bg-emerald-500/10 text-emerald-100' : 'border border-amber-400/20 bg-amber-500/10 text-amber-100'}`}>{item.covered ? 'å·²ç•™æ¡? : 'å¾…è¡¥è·?}</div></div><div className="mt-2 text-[10px] leading-5 text-gray-500">å¿«ç…§ {item.snapshot_count} æ¡ï¼Œæœ€è¿‘ç•™æ¡£ï¼š{item.last_saved_at ? formatDateTimeLabel(item.last_saved_at) : '-'}</div></div>)}</div></div>
                                <div className="rounded-lg border border-sky-500/20 bg-sky-950/10 p-3 space-y-3 text-[11px] text-gray-300"><div><div className="text-[10px] uppercase tracking-wider text-sky-200">å›ºå®šæ¼”ç¤ºå£å¾„</div><div className="mt-1 text-gray-400">ç…§ç€è¿™é‡Œè®²ï¼ŒçŽ°åœºå°±ä¸ä¼šè·³å±‚ï¼Œä¹Ÿä¸ä¼šæ¼æŽ‰å¯¹æ¯”å’ŒæŽ’æŸ¥å£å¾„ã€?/div></div><div className="rounded-lg bg-black/15 px-3 py-2 leading-5"><div>å»ºè®®ä¸»åœºæ™¯ï¼š{recommendedCoverageScenario?.label ?? 'æš‚æœªç¡®å®š'}</div><div>è®²è§£é‡ç‚¹ï¼š{coverageChecklistItems.find(item => item.isRecommended)?.talk ?? 'å…ˆæŠŠå½“å‰ä¸»ä»¿çœŸç»“æžœè·‘é€šåŽå†å®šè®²è§£å£å¾„ã€?}</div></div><div className="space-y-2">{demoGuideSteps.map((step, index) => <div key={`demo-step-${index}`} className="rounded-lg bg-black/15 px-3 py-2 leading-5 text-gray-300">{step}</div>)}</div><div className="grid grid-cols-1 gap-2"><div className="rounded-lg bg-black/15 px-3 py-2 leading-5">çŽ°åœºå¦‚æžœç»“æžœå’Œåœ°å›¾çœ‹èµ·æ¥å¯¹ä¸ä¸Šï¼Œå…ˆå›žçœ‹â€œè¿è¡Œç»“æžœæ‘˜è¦â€å’Œâ€œè¦†ç›–ç»“æžœæ¸…å•â€ï¼Œç¡®è®¤æ˜¯ä¸æ˜¯åœºæ™¯æ²¡ç•™æ¡£æˆ–æ˜ å°„æ²¡å‘½ä¸­ã€?/div><div className="rounded-lg bg-black/15 px-3 py-2 leading-5">å¤±è´¥å…ˆæŸ¥å“ªå±‚ï¼š{failureInvestigationLayerLabel}</div></div></div>
                            </div>
                        )}
