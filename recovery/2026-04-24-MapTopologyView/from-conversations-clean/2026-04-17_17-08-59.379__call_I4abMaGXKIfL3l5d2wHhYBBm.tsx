...
const MapTopologyView: React.FC = () => {
    ...
    useEffect(() => { modeRef.current = editMode; ptRef.current = pointType }, [editMode, pointType])…39881 tokens truncated…f Error ? error.message : 'WE1 涓讳豢鐪熻繍琛屽け璐?) } }} disabled={steadySimLoading} className="rounded-lg bg-cyan-700/80 px-3 py-2 text-xs font-medium text-white disabled:opacity-40">{steadySimLoading ? '杩愯涓?..' : '杩愯涓讳豢鐪?}</button>
                                    ...
                                    <div className="rounded-lg border border-white/10 bg-black/10 px-3 py-2 text-[11px] text-slate-300">
                                        <span className={`inline-block rounded-full border px-2 py-0.5 text-[10px] mr-2 ${steadyStatusToneClass}`}>{steadySimLoading ? '姹傝В涓? : steadyOverlay ? SOLVER_STATUS_LABELS[steadyOverlay.solver_status] : '绛夊緟杩愯'}</span>
                                        <span>鐘舵€侊細{steadyOverlayStatusText}</span>
                                ...
                                <div className="flex items-center justify-between gap-3 text-xs">
                                    ...
                                    <span className="text-emerald-100 font-medium">
                                        {selectedNodeSimulationMetrics?.pressure != null
                                            ? `杩涚珯 ${(selectedNodeSimulationMetrics.pressure * 0.95).toFixed(2)} MPa`
