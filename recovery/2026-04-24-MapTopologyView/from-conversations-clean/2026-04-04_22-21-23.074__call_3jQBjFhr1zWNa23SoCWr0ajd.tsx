...
const MapTopologyView: React.FC = () => {
    ...
    const selectedMergeAutoName = useMemo(() => {
        ...
        if (names.length === 1) return `${names[0]}æž¢çº½`
        const preview = names.slice(0, 2).join(' / ')
        return `${preview}${names.length > 2 ? ` ç­?${names.length} ç«™` : ''}æž¢çº½`
    }, [selectedMergeNodes])
    ...
    useEffect(() => {
        ...
        const collaps…17265 tokens truncated…ansition-colors disabled:opacity-40 flex items-center justify-center gap-1">
