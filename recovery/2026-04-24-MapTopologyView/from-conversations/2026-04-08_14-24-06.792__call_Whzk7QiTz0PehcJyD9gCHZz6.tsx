...
const MapTopologyView: React.FC = () => {
    ...
    const isRuntimeReadonlyGroup = useCallback((group: JunctionGrou…26169 tokens truncated…div className={`mt-1 text-sm font-semibold ${item.tone}`}>{item.value}</div>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                <div className="rounded-lg border border-cyan-500/20 bg-cyan-950/10 p-3 space-y-3">
                                    <div className="text-[10px] uppercase tracking-wider text-cyan-200">ä¸»ä»¿çœŸæ“ä½?/div>
                                    <div className="grid grid-cols-1 gap-2">
                                        <div>
                                            <div className="mb-1 text-[10px] text-gray-500">å½“å‰åœºæ™¯</div>
                                        ...
                                        <div>
                                            <div className="mb-1 text-[10px] text-gray-500">ç»“æžœå¿«ç…§</div>
                                            <div className="flex gap-2">
                                                <select value={steadySelectedSnapshotRunId} onChange={event => setSteadySelectedSnapshotRunId(event.target.value)} className="flex-1 rounded-lg border border-cyan-500/20 bg-black/20 px-3 py-2 text-xs text-white outline-none">
                                                    <option value="">è¯·é€‰æ‹©ä¸€æ¡å¿«ç…?/option>
                                        ...
                                        <div>
                                            <div className="mb-1 text-[10px] text-gray-500">åŸºçº¿å¿«ç…§</div>
                                            <select value={steadyBaselineSnapshotRunId} onChange={event => setSteadyBaselineSnapshotRunId(event.target.value)} className="w-full rounded-lg border border-violet-500/20 bg-black/20 px-3 py-2 text-xs text-white outline-none">
                                                <option value="">è¯·é€‰æ‹©åŸºçº¿å¿«ç…§</option>
                                    ...
                                    <div className="rounded-lg border border-white/10 bg-black/10 px-3 py-2 space-y-2">
                                        <div className="text-[10px] uppercase tracking-wider text-slate-300">å¿«æ·æ“ä½œ</div>
                                        <div className="grid grid-cols-1 gap-2">
                                            <button onClick={applyRecommendedScenarioQuickSwitch} disabled={!recommendedCoverageScenario} className="rounded-lg border border-emerald-500/30 bg-emerald-900/10 px-3 py-2 text-xs text-emerald-100 disabled:opacity-40">
                                                ä¸€é”®åˆ‡åˆ°æŽ¨èåœºæ™¯ï¼š{recommendedCoverageScenario?.label ?? 'æš‚æ— '}
                                            </button>
                                            <div className="grid grid-cols-2 gap-2">
                                                <button onClick={applySelectedSnapshotAsBaseline} disabled={!steadySelectedSnapshotRunId} className="rounded-lg border border-violet-500/30 bg-violet-900/10 px-3 py-2 text-xs text-violet-100 disabled:opacity-40">å½“å‰å¿«ç…§è®¾åŸºçº?/button>
                                                <button onClick={applyLatestCoveredSnapshotAsBaseline} disabled={!latestCoveredSnapshotSummary} className="rounded-lg border border-sky-500/30 bg-sky-900/10 px-3 py-2 text-xs text-sky-100 disabled:opacity-40">æœ€è¿‘ç•™æ¡£è®¾åŸºçº¿</button>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2">
                                        <button onClick={async () => { try { await runSteadySimulation(); setStatusMsg(`WE1 ä¸»ä»¿çœŸå·²è¿è¡Œï¼?{steadyScenarioOption?.label ?? steadyScenarioId}`) } catch (error) { setStatusMsg(error instanceof Error ? error.message : 'WE1 ä¸»ä»¿çœŸè¿è¡Œå¤±è´?) } }} disabled={steadySimLoading} className="rounded-lg bg-cyan-700/80 px-3 py-2 text-xs font-medium text-white disabled:opacity-40">{steadySimLoading ? 'è¿è¡Œä¸?..' : 'è¿è¡Œä¸»ä»¿çœ?}</button>
                                        <button onClick={async () => { try { await saveSteadySnapshot(); setStatusMsg(`å·²ä¿å­˜å¿«ç…§ï¼š${steadyScenarioOption?.label ?? steadyScenarioId}`) } catch (error) { setStatusMsg(error instanceof Error ? error.message : 'ä¿å­˜å¿«ç…§å¤±è´¥') } }} disabled={!steadyOverlay || steadySnapshotLoading} className="rounded-lg border border-sky-500/30 bg-sky-900/20 px-3 py-2 text-xs font-medium text-sky-100 disabled:opacity-40">{steadySnapshotLoading ? 'ä¿å­˜ä¸?..' : 'ä¿å­˜å¿«ç…§'}</button>
                                        <button onClick={async () => { try { await refreshSteadySnapshots(); setStatusMsg('å¿«ç…§åˆ—è¡¨å·²åˆ·æ–?) } catch (error) { setStatusMsg(error instanceof Error ? error.message : 'åˆ·æ–°å¿«ç…§å¤±è´¥') } }} disabled={steadySnapshotLoading || steadyBaselineSnapshotLoading} className="rounded-lg border border-slate-600 bg-slate-800/70 px-3 py-2 text-xs font-medium text-slate-200 disabled:opacity-40">åˆ·æ–°å¿«ç…§</button>
                                        <button onClick={() => { clearSteadyOverlay(); setStatusMsg('å·²æ¸…ç©ºå½“å‰ä¸»ä»¿çœŸç»“æžœ') }} className="rounded-lg border border-slate-600 bg-slate-800/70 px-3 py-2 text-xs font-medium text-slate-200">æ¸…ç©ºä¸»ä»¿çœ?/button>
                                        <button onClick={exportSimulationReport} className="col-span-2 rounded-lg border border-emerald-500/30 bg-emerald-900/20 px-3 py-2 text-xs font-medium text-emerald-100">å¯¼å‡ºæ¼”ç¤ºæ¸…å•</button>
                                    </div>
                                </div>
