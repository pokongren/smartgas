 ...
 *
 * åªæ˜¾ç¤ºçº¯æ‹“æ‰‘å±‚ï¼ˆç®€æ´åœ†ç‚?+ ç›´çº¿ï¼‰ï¼Œéšè—æ‰€æœ‰ç«™åœ?é˜€å®¤çš„å¤æ‚å›¾å½¢ã€?
 * ç‹¬ç«‹äºŽå…¨å›½ç®¡ç½‘ç»Ÿä¸€è§†å›¾å’?Canvas æ‹“æ‰‘è§†å›¾ã€?
 */
...
const TYPE_MAP: Record<string, PointType> = {
    ...
    valve: 'valve',
    junction: 'station',
    // NOTE: sj4 ç­‰æ–°æ•°æ®æ–‡ä»¶ç›´æŽ¥ä½¿ç”¨ compressor/distribution ç­‰å€?
...
const MapTopologyView: React.FC = () => {
    ...
    // åˆå»ºç«™ï¼šåˆå¹¶æ¨¡å¼çŠ¶æ€?    const [selectedMergeNodeIds, setSelectedMergeNodeIds] = useState<string[]>([])
    ...
    const loadJunctionGroups = useCallback(async () => {
        try {
            const groups = await topologyEditorApi.getJunctionGroups()
            setJunctionGroups(groups)
            return groups
        } catch (error) {
    ...
    useEffect(() => {
        ...
        if (editMode === 'draw-point') setStatusMsg(`ç»˜åˆ¶ï¼šç‚¹å‡»åœ°å›¾æ·»åŠ ã€?{TOPO_LABELS[pointType]}ã€`)
        else if (editMode === 'connect') { setStatusMsg('è¿žçº¿ï¼šç‚¹å‡»èµ·ç‚¹èŠ‚ç‚?); setConnectFrom(null) }
        else if (editMode === 'merge') { setStatusMsg('æåˆï¼šç‚¹å‡»èŠ‚ç‚¹è¿›è¡Œå¤šé€‰ï¼Œå†ç‚¹å‡»ã€Œåˆ›å»ºæž¢çº½ã€?); setConnectFrom(null) }
        else setStatusMsg('æµè§ˆæ¨¡å¼')
    ...
    const doAddNode = (lnglat: any, type: PointType) => {
        ...
        setStatusMsg(`å·²æ·»åŠ? ${node.name}`)
    }

    /** æ‹–æ‹½åŽåŒæ­¥å…³è”ç®¡çº?*/
    ...
    const toggleMergeSelection = useCallback((nodeId: string) => {…13706 tokens truncated…l flex items-center gap-2 p-1.5 rounded text-xs hover:bg-white/5 text-left">
                                ...
                                {cutoffNodeId && (
                                    <div className="bg-red-900/20 border border-red-700/40 rounded-lg p-2">
                                        <p className="text-[10px] text-red-400 mb-0.5">æˆªæ–­ç‚?/p>
                                ...
                                <div className="flex gap-1.5">
                                    <button
                                        ...
                                        <span className="material-symbols-outlined text-sm">play_arrow</span>è¿è¡Œä»¿çœŸ
                                    </button>
                                    <button
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        {cutoffResult.sourceNodes.length > 0 && (
                                            <div className="bg-blue-900/15 border border-blue-700/30 rounded-lg p-2 text-[10px]">
                                                <p className="text-blue-400 mb-1">è¯†åˆ«åˆ?{cutoffResult.sourceNodes.length} ä¸ªæ°”æº?/p>
                                                <div className="text-gray-400 space-y-0.5 max-h-16 overflow-y-auto">
                                                    {cutoffResult.sourceNodes.map(s => (
                                                        <div key={s.id} className="truncate">Â· {s.name}</div>
                                                    ))}
                                                </div>
                                            </div>
                                        )}
                                        ...
                                        <div className="bg-gray-900/60 border border-gray-700/30 rounded-lg p-2.5 text-[10px]">
                                            <p className="text-gray-400 mb-1.5">åˆ†æžäº?{cutoffResult.totalDistributionNodes} ä¸ªåˆ†è¾“ç«™</p>
                                            <div className="flex gap-2">
                                                ...
                                                <span className="flex-1 bg-orange-900/30 rounded px-2 py-1 text-center">
                                                    ...
                                                    <span className="text-gray-500">ç»•è¡Œ</span>
                                                </span>
                                        ...
                                        {cutoffResult.affectedNodes.length > 0 && (
                                            <div className="space-y-0.5 max-h-52 overflow-y-auto">
                                                <p className="text-[10px] text-gray-500 mb-1">å—å½±å“èŠ‚ç‚?/p>
                                                    ...
                                                    <div key={node.id}
                                                        ...
                                                        <div className="flex items-center gap-1.5">
                                                            ...
                                                            <span className={node.status === 'supply_lost' ? 'text-red-400' : 'text-orange-400'}>
                                                                {node.status === 'supply_lost' ? 'æ–­ä¾›' : 'ç»•è¡Œ'}
                                                            </span>
                                                        </div>
                                                        {/* è·¯å¾„é¢„è§ˆ */}
                                                        {node.status === 'rerouted' && node.pathAfter.length > 0 && (
                                                            <div className="mt-1 text-[9px] text-gray-600 truncate pl-4">
                                                                â¤?{pathIdsToNames(node.pathAfter, topoNodes.map(n => ({ id: n.id, name: n.name, type: n.type }))).join(' â†?')}
                                                            </div>
                                                        )}
                                                        {node.status === 'supply_lost' && node.pathBefore.length > 0 && (
                                                            <div className="mt-1 text-[9px] text-gray-600 truncate pl-4">
                                                                âš?åŽŸè·¯å¾? {pathIdsToNames(node.pathBefore, topoNodes.map(n => ({ id: n.id, name: n.name, type: n.type }))).join(' â†?')}
                                                            </div>
                                                        )}
                                                    </div>
                                                ))}
                                ...
                                {/* æç¤º */}
                                {topoNodes.length === 0 && (
                                    <p className="text-center text-gray-600 text-[10px] py-4">è¯·å…ˆç‚¹å‡»ã€Œå¯¼å…¥æ‹“æ‰‘ã€?/p>
                                )}
                            </div>
                        )}
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 text-sm">
                            ...
                            {selectedNode.isJunction ? (
                                <div className="rounded-lg border border-purple-500/20 bg-purple-500/5 p-3 space-y-3">
                                    ...
                                    >
                                        <span className="material-symbols-outlined text-sm">device_hub</span>è§£é™¤æåˆ
                                    </button>
                                    <button
                                ...
                                <div className="rounded-lg border border-gray-700/40 bg-white/5 p-3">
                                    ...
                                    >
                                        <span className="material-symbols-outlined text-sm">hub</span>åŠ å…¥å¾…æåˆåˆ—è¡?
                                    </button>
                                </div>
                            )}
                        </div>
