...
const MapTopologyView: React.FC = () => {
    ...
    useEffect(() => {
        ...
        else if (editMode === 'merge') { setStatusMsg('鎹忓悎锛氱偣鍑昏妭鐐硅繘琛屽閫夛紝鍐嶇偣鍑汇€屽垱寤烘灑绾姐€?); setConnectFrom(null) }
    ...
    const toggleMergeSelection = useCallback((nodeId: string) => {
        ...
        setSelectedMergeNodeIds(prev => {
            ...
            setStatusMsg(`宸查€夋嫨 ${next.length} 涓緟鎹忓悎鑺傜偣`)
            return next
        })
    }, [])
    ...
    const doNodeClick = (nodeId: string, event?: any) => {
        ...
        if (modeRef.current === 'connect') {
            ...
            if (!from) {
                ...
                setStatusMsg(`璧风偣銆?{nd?.name}銆嶁啋 鐐瑰嚮缁堢偣`)
            } else {
                if (nodeId === from) { setStatusMsg('涓嶈兘杩炴帴鑷韩'); return }
                doAddEdge(from, nodeId)
                setConnectFrom(null)
                setStatusMsg('杩炵嚎鎴愬姛锛岀户缁偣鍑昏捣鐐?)
            }
            return…12536 tokens truncated…                             cutoffSearch
                                            ).map(node => (
                                                <button key={node.id}
                                                    onClick={() => {
